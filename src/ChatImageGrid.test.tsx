...
describe('ChatImageGrid', () => {
  it('renders correct number of cells', () => {
    render(<ChatImageGrid items={mockItems} />)
  ...
  it('calls onMediaClick with index and item', () => {
    const onClick = vi.fn()
    render(<ChatImageGrid items={mockItems} onMediaClick={onClick} />)
    const cells = document.querySelectorAll('.chat-image-cell')
  ...
  it('renders nothing for empty items', () => {
    const { container } = render(<ChatImageGrid items={[]} />)
    expect(container.firstChild).toBeNull()
  })

  it('applies custom className', () => {
    render(<ChatImageGrid items={mockItems} className="custom" />)
    expect(document.querySelector('.chat-image-grid.custom')).toBeTruthy()
  })

  it('applies grid dimensions from layout', () => {
    render(<ChatImageGrid items={[mockItems[0]]} maxWidth={400} />)
  ...
  it('cells have accessible role and tabIndex', () => {
    render(<ChatImageGrid items={[mockItems[0]]} />)
  ...
  it('handles keyboard navigation', () => {
    const onClick = vi.fn()
    render(<ChatImageGrid items={[mockItems[0]]} onMediaClick={onClick} />)
    const cell = document.querySelector('.chat-image-cell') as HTMLElement
